...
async function run() {
  try {
    ...
    const customerNames = [
      "Nike India", "Netflix India", "Tech Corp", "Adobe Creative", "Flipkart Studios",
      "Amazon Prime", "Airbnb India", "Uber India", "Swiggy", "Zomato",
      "ICICI Bank", "HDFC Insurance", "Hindustan Unilever", "ITC Limited", "Nestlé India",
      "Hero MotoCorp", "Bajaj Auto", "Maruti Suzuki", "Hyundai India", "MG Motors",
      "Apple India", "Samsung India", "OnePlus India", "Realme India", "Poco India",
      "Myntra", "Nykaa", "Unacademy", "Byju's", "PhonePe",
    ];

    const locations = [
      "Film City", "ND Studio", "NSIC Grounds", "Studio A", "Studio B",
      "Filmistan", "EastEnd Studio", "Lonavala", "Alibaug", "Bandra",
      "Colaba", "Powai", "Andheri", "Malad", "Borivali",
    ];
    ...
    for (let i = 0; i < dummyBookings.length; i += batchSize) {
      ...
      console.log(`Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(dummyBookings.length / batchSize)}`);
    }
