    ...
    <div className="w-full">
      ...
      <div className="hidden md:grid grid-cols-7 gap-1">
        ...
        {calendarDates.map((date, idx) => {
          ...
          const style = idx === 0 ? { gridColumn: startingDayOfWeek + 1 } : undefined;
          ...
          return (
            <div
              ...
              `}
            >
              <div className={`text-xs sm:text-sm font-bold ${isToday ? "text-[#e63030]" : "text-[#f0ede8]"}`}>
                {date.getDate()}
              </div>
              {daybookings && daybookings.length > 0 && (
                <div className="flex items-center gap-0.5 flex-wrap mt-1">
                  ...
                  {daybookings.length > 3 && (
                    <span className="text-xs font-semibold text-[#e63030]">+{daybookings.length - 3}</span>
                  )}
                </div>
              )}
            </div>
        ...
        {calendarDates.map((date, idx) => {
          ...
          return (
            <div
              ...
              `}
            >
              <div className="text-xs font-medium text-[#888888]">{dayName}</div>
              <div className={`text-xs font-bold ${isToday ? "text-[#e63030]" : "text-[#f0ede8]"}`}>
                {date.getDate()}
              </div>
              {daybookings && daybookings.length > 0 && (
                <div className="flex items-center gap-0.5 flex-wrap mt-0.5">
                  ...
                  {daybookings.length > 2 && (
                    <span className="text-[10px] font-semibold text-[#e63030]">+{daybookings.length - 2}</span>
                  )}
                </div>
              )}
            </div>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <p className="font-semibold text-sm text-[#f0ede8]">{booking.customerName}</p>
                    <p className="text-xs text-[#888888]">
                      {booking.startTime} - {booking.endTime}
                    </p>
                    {booking.serviceName && (
                      <p className="text-xs text-[#888888]">{booking.serviceName}</p>
                    )}
                    {booking.city && (
                      <p className="text-xs text-[#888888]">{booking.city}</p>
                    )}
                  </div>
