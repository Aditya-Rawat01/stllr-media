...
import { leads, bookings, services } from "@/lib/db/schema";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (err.message?.includes("column") || err.message?.includes("status")) {
        return NextResponse.json(
          {
            error: "Migration pending",
            details: "Lead status column not yet added to database. Run this SQL in Neon console: CREATE TYPE \"public\".\"lead_status\" AS ENUM('not_contacted', 'in_progress', 'converted'); ALTER TABLE \"leads\" ADD COLUMN \"status\" \"lead_status\" DEFAULT 'not_contacted' NOT NULL;",
          },
          { status: 503 }
        );
      }
