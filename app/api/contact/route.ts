...
const schema = z.object({
  ...
  enquiryType: z.enum(["general", "videography", "photography", "video_editing", "drone", "combo", "brand_campaign", "corporate"]).default("general"),
});
...
export async function POST(req: Request) {
  ...
  let leadId: string;
  try {
    const rows = await db.insert(leads).values({ email, phone, enquiryDetails, enquiryType }).returning({ id: leads.id });
    leadId = rows[0].id;
  } catch (e: any) {
    return Response.json({ error: "DB error", details: e.message }, { status: 500 });
  }

  // 2. send mails via Resend (if key present, else skip)
  const apiKey = process.env.RESEND_API_KEY;
  const from = process.env.RESEND_FROM || "Stllr Media <onboarding@resend.dev>";
...
export async function GET() {
  return Response.json({ status: "ok", hint: "POST {email, phone, enquiryDetails, enquiryType}" });
}
